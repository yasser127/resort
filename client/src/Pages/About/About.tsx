...
import HeroImg from "../../assets/My-Nature-Resort.webp";
...
function makeArray(v: unknown) {
  ...
  if (typeof v === "string") {
    ...
    if (!trimmed) return [];
    // try to detect JSON array encoded as string
    try {
  ...
  return (
    ...
    >
      <div className="text-3xl md:text-4xl font-extrabold text-gray-900">
        <AnimatedNumber to={value} duration={2} suffix={suffix} start={inView} />
      </div>
...
export default function AboutPage() {
  // fetched content (fallbacks keep original copy)
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (remoteDesc && String(remoteDesc).trim()) setDescription(String(remoteDesc));
        if (remoteStory && String(remoteStory).trim()) setStory(String(remoteStory));

        // singularities: prefer array; if string, try parse JSON array else single-item
        const singArr = makeArray(remoteSing);
        if (singArr.length > 0) {
          // map to decorated tags (if user stored plain text, keep it)
          setSingularities(singArr.map((s) => (s.startsWith("✨") || s.startsWith("🏆") || s.startsWith("🌱") ? s : s)));
        }

        if (remoteMission && String(remoteMission).trim()) setMission(String(remoteMission));
        if (remoteValues && String(remoteValues).trim()) setValues(String(remoteValues));
        if (remoteSignature && String(remoteSignature).trim()) setSignatureExperience(String(remoteSignature));

        // stats - convert safely to numbers with fallback
    ...
    <div className="min-h-screen font-serif text-gray-900">
      ...
      <header className="absolute top-6 left-6 right-6 z-30 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-md bg-black/80 text-white flex items-center justify-center font-bold">LR</div>
        ...
        <div>
          <button className="hidden lg:inline-block bg-white text-black rounded-md px-4 py-2 text-sm shadow">Book Now</button>
        </div>
      </header>
      ...
      <section
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(6,6,6,0.55), rgba(6,6,6,0.55)), url(${HeroImg})`,
        }}
      >
        <div className="container mx-auto px-6 lg:px-12">
          <div className="max-w-3xl text-white">
            ...
            <p className="mt-6 text-base md:text-lg text-white/90">{description}</p>
            <div className="mt-8 flex gap-4">
              <a href="#book" className="inline-block bg-transparent border border-white/60 text-white px-4 py-2 rounded-md text-sm">Book a Room</a>
              <a href="#services" className="inline-block bg-white text-black px-4 py-2 rounded-md text-sm">View Services</a>
            </div>
          </div>
        </div>
      </section>
      ...
      <main className="-mt-16 relative z-20">
        <div className="container mx-auto px-6 lg:px-12">
          {/* Our Story */}
          <section id="about" className="bg-white rounded-xl shadow-lg p-8 md:p-12 -mt-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                <h2 className="text-3xl font-bold">Our Story</h2>
                <p className="mt-4 text-gray-700 leading-relaxed whitespace-pre-line">{story}</p>
                <div className="mt-6 flex flex-wrap gap-3">
                  {singularities.map((s, i) => (
                    <span key={i} className="inline-flex items-center gap-2 px-3 py-2 w-[30%]  rounded-md bg-gray-50 border">
                      {s}
                    </span>
                  ))}
                </div>
              ...
              <div className="rounded-lg overflow-hidden">
                <img src={HeroImg} alt="Resort interior" className="w-full h-72 object-cover md:h-96 rounded-lg shadow" />
              </div>
            </div>
          </section>

          {/* Mission & Values */}
          <section id="mission" className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="p-6 bg-white rounded-xl shadow">
              ...
              <ul className="mt-3 text-gray-600 space-y-2">
                {String(values).split(/[;,•]/).filter(Boolean).map((v, i) => (
                  <li key={i}>• {v.trim()}</li>
                ))}
              </ul>
            </div>
          ...
          <section className="mt-12 p-8 rounded-xl bg-gradient-to-r from-white/60 via-white to-gray-50 shadow-lg">
            <div className="container mx-auto px-0 md:px-4 flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex gap-8 w-full md:w-auto">
                <Stat title="Rooms & Suites" value={stat1} suffix="+" delay={0.0} />
                <Stat title="Experiences Curated" value={stat2} suffix="+" delay={0.15} />
                <Stat title="Guest Satisfaction" value={stat3} suffix="%" delay={0.3} />
              </div>

              <div className="w-full md:w-1/3">
                <div className="text-right md:text-left">
                  <h4 className="text-xl font-semibold">Ready to experience Luxury Resort?</h4>
                  <p className="text-gray-600 mt-2">Book your stay or get in touch with our concierge to begin planning.</p>
                  <div className="mt-4 flex justify-end md:justify-start">
                    <a className="inline-flex items-center gap-3 bg-black text-white px-5 py-3 rounded-full shadow-md transform hover:-translate-y-0.5 transition" href="#book">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden>
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11.5V11a1 1 0 01-2 0V6.5a1 1 0 012 0zM10 13a1.5 1.5 0 100-3 1.5 1.5 0 000 3z" clipRule="evenodd" />
                      </svg>
                      Reserve
                    </a>
                  </div>
          ...
          <section className="mt-16 mb-24 p-1 rounded-2xl bg-gradient-to-r from-purple-600 via-pink-600 to-amber-400">
            <div className="rounded-2xl bg-black/95 text-white p-8 md:p-12 shadow-2xl">
              <div className="max-w-4xl mx-auto text-center">
                <motion.h4 initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-2xl md:text-3xl font-semibold">
                  Experience timeless luxury
                </motion.h4>

                <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: 0.12, duration: 0.6 }} className="mt-3 text-white/90 max-w-2xl mx-auto">
                  Contact our concierge to create a bespoke stay tailored to you. We’ll handle everything from private transfers to personalized menus.
                </motion.p>

                <div className="mt-6 flex items-center justify-center gap-4">
                  <motion.a href="#contact" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} className="inline-flex items-center gap-3 bg-white text-black px-6 py-3 rounded-full font-medium shadow-lg">
                    Contact Concierge
                  </motion.a>

                  <motion.a href="#book" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} className="inline-flex items-center gap-2 border border-white/20 text-white px-5 py-3 rounded-full font-medium hover:bg-white/5 transition">
                    View Rooms
                  </motion.a>
                </div>
