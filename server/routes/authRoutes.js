...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    const [existing] = await pool.query("SELECT id FROM `user` WHERE email = ? LIMIT 1", [email]);
...
router.get("/me", verifyToken, async (req, res) => {
  try {
    const pool = await getPool();
    const [rows] = await pool.query("SELECT id, name, email FROM `user` WHERE id = ? LIMIT 1", [req.id]);
